...
-   **`src/components/UnifiedLiquidRail.css`**: Stylesheet for the **Unified Liquid Rail**. **Modified in this task** to enable scrolling by adding `overflow-y: auto` and to improve the opening animation by setting `transform-origin: top center`.
...
-   **`src/components/UnifiedLiquidRail.jsx`**: The primary editor toolbar. **Modified in this task** to resolve all outstanding UI glitches. The `handleAction` call was moved to the `onPointerDown` handler to fix a critical focus-related bug preventing styles from being applied. The toolbar's configuration was also corrected to fix state corruption and add the missing list tools to the "Add" mode.
