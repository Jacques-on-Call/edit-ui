...
-   **`src/components/UnifiedLiquidRail.css`**: Stylesheet for the **Unified Liquid Rail**. **Modified in this task** to implement the "Micro-Capsule" design blueprint. This includes the 44px width, the 88px tall "pill" trigger, the translucent glass effect, the "weld" logic for seamless opening, and the keyboard-safe `max-height`. It also hides tool labels and centers icons in the compact view.
...
-   **`src/components/UnifiedLiquidRail.jsx`**: The primary editor toolbar. **Modified in this task** to align with the "Micro-Capsule" design. The CSS was updated to hide tool labels and center icons in the compact view, resulting in a sleeker, 44px wide toolbar that improves content visibility during editing.
