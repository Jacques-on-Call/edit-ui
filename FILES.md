...
-   **`src/components/UnifiedLiquidRail.css`**: The stylesheet for the **Unified Liquid Rail**. Contains all styling for the `position: fixed` overlay, the "glasslike" translucent effect (`backdrop-filter`), and the compact/expanded states.
...
-   **`src/components/UnifiedLiquidRail.jsx`**: The primary editor toolbar. Renders as a `position: fixed` overlay anchored to the top-left of the viewport. It features an always-visible hamburger icon. Text selection reveals 'style' tools, a single tap on the hamburger reveals 'add' tools, and a double-tap expands the rail to show text labels. It is rendered via a portal and does not affect the page's layout.
