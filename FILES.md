...
-   **`src/components/SlideoutToolbar.jsx`**: The **Unified Liquid Rail**. This is the primary toolbar for the editor. It is a translucent, context-aware component that handles both "add" actions (via hamburger tap) and "style" actions (appears automatically on text selection). It features a two-stage expansion (icons-only, then icons with labels) to save screen space.
-   **`src/components/SidePanelToolbar.jsx`**: DEPRECATED - This component was the old, opaque styling toolbar that covered content. It has been replaced by the unified `SlideoutToolbar.jsx`.
