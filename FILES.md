...
-   **`src/components/UnifiedLiquidRail.css`**: The stylesheet for the **Unified Liquid Rail**. **Modified in this task** to support the new, architecturally separate hamburger trigger. It now defines a `rail-hamburger-trigger` class for the always-visible button and sets the main rail's background transparency to 30%.
...
-   **`src/components/UnifiedLiquidRail.jsx`**: The primary editor toolbar. **Modified in this task** to be architecturally separated from its trigger. The component now renders a portal containing two distinct elements: an always-visible hamburger button (`.rail-hamburger-trigger`) and the toolbar panel itself, which fades in and out. The internal logic has been simplified to make the hamburger the universal entry point for the "Add" mode.
