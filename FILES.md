...
-   **`src/components/UnifiedLiquidRail.jsx`**: The **Unified Liquid Rail (Architect-Approved)**. This is the primary, stateful toolbar for the editor. It appears as a fixed hamburger icon and expands into a "compact" (style) or "expanded" (add) state. It is rendered via a portal to ensure correct viewport positioning and communicates its width to the parent `EditorCanvas` to enable dynamic content padding.
