...
-   **`src/components/UnifiedLiquidRail.css`**: The stylesheet for the **Unified Liquid Rail**. **Modified in this task** to support the new, architecturally separate hamburger trigger. It now defines a `rail-hamburger-trigger` class for the always-visible button and sets the main rail's background to a translucent black.
...
-   **`src/components/UnifiedLiquidRail.jsx`**: The primary editor toolbar. **Modified in this task** to resolve multiple UI glitches. The component is now architecturally separated from its trigger. It renders an always-visible hamburger button for the "Add" mode and a separate toolbar panel that appears for "Style" mode. Event listeners were refined to be more reliable, and the click-outside logic now correctly accounts for the separate trigger.
