...
-   **`src/components/SlideoutToolbar.css`**: The stylesheet for the **Unified Liquid Rail**. Contains all styling for the refined, architect-approved "liquid glass" effect, the integrated hamburger menu, and the responsive layout for both compact (icon-only) and expanded states.
...
-   **`src/components/SlideoutToolbar.jsx`**: The **Unified Liquid Rail (Architect-Approved)**. This is the primary toolbar for the editor. It uses a critical `onPointerDown` event handling strategy to prevent the editor from losing focus, which resolves all UI glitches and style application failures. It is a translucent, context-aware component that handles both "add" and "style" actions.
-   **`src/components/SidePanelToolbar.jsx`**: DEPRECATED - This component was the old, opaque styling toolbar that caused numerous focus and usability issues. It has been completely replaced by the architect-approved `SlideoutToolbar.jsx`.
